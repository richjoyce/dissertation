...
In this experiment, the subjects were asked to target a set of buttons multiple times in various levels of virtual reality and the real world.
...
Significant recent work investigates virtual reality tools in a simulator \citep{wan_mrstudio:_2011, yavrucuk_low_2011, aslandere_virtual_2015}.
...
As previously described (Chapter \ref{sec:proto_button_input}), the hand tracker mounted above the panel enables detection of when the user presses a button without need for a touch sensor.
...
\citet{aslandere_virtual_2015} found no significant effect with button selection ability based on changing this volume in a purely virtual environment, so we determined an approriate size in pilot studies and did not include the size of the detection box as a variable.
