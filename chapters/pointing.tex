...
Specifically, we were interested in whether an untrained user could accurately target the correct button in an instrument panel, and how the different technologies used affected the targeting task in time and accuracy.
...
A motion was recorded as successful if the subject pressed the correct button within the 10~second timeout and it was the first button they pressed.
...
Furthermore, the time recorded of the tasks were typically more than in a Fitts' task, since many subjects spent time honing their performance of the accuracy based on the instructions.
...
In this experiment, subjects only targeted one button on the panel before returning to the start pads.
...
Subjective data was not collected from the participants of this study, so formal preferences and opinion of the subjects were not recorded.
...
Overall, subjects spent more time to select buttons in the virtual reality conditions for our targeting task, but just as accurately and successfully.
