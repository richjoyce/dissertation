...
The age range was 19-29 ($M=23.0, \sigma=3.0$) with 16 males and 4 females.
...
The first metric was the ratio of path distance traveled in the ballistic phase to the distance between the targets for that movement (i.e. 20cm, 30cm or 40cm).
...
For this metric it was compared to the total path distance the subject traveled for their entire movement.
...
The threshold for the filter was set at 0.80, leaving 4264 movements lower than the threshold, though only 1675 were unique from the previous target distance filter.
...
A post-hoc repeated measures t-test between haptic conditions for the subjects who performed PH First was significant ($t(9)=4.62, p<0.001$), with the PH condition ($M=4.23, \sigma=0.34$) outperforming the NH condition ($M=3.91, \sigma=0.40$).
The mean of the differences between subjects was 0.32 bps.
The group of subjects who performed NH first also had a significant effect in the t-test ($t(9)=3.96, p<0.001$) between the PH condition ($M=4.28, \sigma=0.54$) and the NH condition ($M=3.62, \sigma=0.32$).
