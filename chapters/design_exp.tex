...
The choice of a touchscreen-based simulator as the baseline tool was motivated by two reasons: feasibility and appropriateness.
...
%Touchscreen devices have been used 
%The goal is not to necessarily replace , but to provide an alternative tool.
...
For the evaluation sessions, subjects performed six trials with both tasks for each design.
...
The effect of Group yielded an $F$ ratio of $F(1, 21) = 21.4, p < 0.001$ indicating a significant difference between VR ($M=\ang{1.97}, \sigma=\ang{0.38}$) and TS ($M=\ang{1.28}, \sigma=\ang{0.38}$).
The TS group had a lower RMSE for the tracking task.
In both groups, subjects were performing the tracking task using their left hand to control the joystick.
The effect of Design was marginally significant with an $F$ ratio of $F(1,21) = 5.94, p = 0.024$.
The tracking task was performed better with the Keypad design ($M=\ang{1.57}, \sigma=\ang{0.51}$) than with the Edgekey design ($M=\ang{1.70}, \sigma=\ang{0.52}$).
There was no difference between designs with the attitude indicator display, except for a small shift in location.
...
We also report here the results of the subjects during the trials where they were only performing the tracking task.
The subjects ran a single trial that consisted of just the tracking task without the prompting task at the end of each evaluation session.
...
These results highlight the difference in performance when the subject did not have the additional prompting task workload.
...
These are reported to help understand the results, but are not used in the statistical tests.
...
The TS group had a marginally significant difference ($t(10) = 2.28, p = 0.045$) between the Keypad ($M = 88.4\%, \sigma = 8.0\%$) and the Edgekey ($M = 81.6\%, \sigma = 11.7\%$).
...
The difference between groups and designs is largely due to subjects not being able to complete the prompt.
...
Within the VR group the Edgekey design caused 52.3\% of prompts to be incomplete before the timeout occurred, compared to 18.8\% using the Keypad design.
The TS group also had more incomplete prompts in the Edgekey design (9.4\%) than in the Keypad design (1.0\%).
However, the Keypad design was performed with almost no incomplete prompts with the TS group.
...
Their scores, which are weighted means by the pairwise comparisons, are used here as a measure of their self-reported workload.
The results by Group and Design are shown in Figure~\ref{fig:de_factor_tlx}.
...
The repeated measures t-tests indicated significance between designs for the VR group ($t(11) = -4.20, p = 0.001$) between the Keypad design ($M = 54.4, \sigma = 20.4$) and the Edgekey design ($M = 85.6, \sigma = 11.2$)
...
\begin{figure}
    ...
    \caption{RMSE for Tracking Only Trials by Group and Trial. Error bars are standard error of the mean. Trial 1 was final trial of RMSE training, Trial 2 was after first design evaluation, Trial 3 was after second design evaluation. Design was not included as the tracking task display did not change between designs.}
...
Motion sickness was not noted by any of the subjects.
...
The prompt completion accuracy was found to be significant in Group and Design.
...
The results of the NASA-TLX subject perceived workload score found an effect in Design, but no effect in Group.
