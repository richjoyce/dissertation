...
The flight condition is listed as ``Flight Condition 2'' in NASA CR-2044\cite{NASACR}\tinytodo{reference}.
...
The disturbance model is based off the model developed in SweetRef\cite{sweet}\tinytodo{reference}.
...
\begin{figure}
    ...
    \begin{tikzpicture}[auto]
        ...
        \node [draw=none, text width=7em, below=3cm of Keypad] (targetjoin) {};
        ...
        \node [draw=none, above=2cm of nloop1, text width=6em, text centered] (nloop) {Repeat for 4 Characters};

        \node [decision, right=1.5cm of targetjoin] (decide) {Are labels for $n^{\mathrm{th}}$ character shown?};
        \node [cloud] (Edgekey) at (Keypad -| decide) {Edgekey};
        ...
        \path [line] (recall) -- (Keypad);
        \path [line] (Keypad) -- (target);
        \path [line] (target) -- (press);
        \path [line,-] (press) -| (nloop);
        \path [line] (nloop) |- (recall);

        \path [line, dashed] (recall.south) ++(0,-0.5cm) -| (Edgekey);
        \path [line, dashed] (Edgekey) -- (decide);
        \path [line, dashed] (decide) -- node [above] {Yes} (targetjoin.center);
        \path [line, dashed] (decide) -- node {No} (targetswitch);
        \path [line, dashed] (targetswitch) -- (pressswitch);
        \path [line, dashed] (pressswitch) -- (recall2);
        \path [line, dashed] (recall2.east) -- ++(1.5cm,0) |- (decide.east);

    \end{tikzpicture}
    \medskip
    \caption{Prompting Task Flowchart of Cognitive Work for Each Design. Extra work of Edgekey design shown with dashed lines.}
...
This consisted of 885 of the total 1700 prompts recorded for the Edgekey design.
