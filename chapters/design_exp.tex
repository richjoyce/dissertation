...
One factor that could influence the response time between groups is the additional time to activate a button in the VR environment versus the touchscreen.\todo{explain this?}
...
The TS group had a marginally significant difference ($t(10) = 2.28, p = 0.045$) between Keypad ($M = 9.82, \sigma = 1.38$) and the Edgekey ($M = 10.6, \sigma = 0.96$).\todo{these are backward}
...
However, it appears that they had difficulty in both designs compared to the TS group.

This agrees with the post-hoc tests for differences in design between groups.
These had significant effects for both the Keypad design ($t(21) = 4.44, p < 0.001$) between the VR group and the TS group, and the Edgekey design ($t(21) = 7.05, p < 0.001$) between the VR group and the TS group.
The main effect of group clearly has a meaningful effect, as the VR group ($M = 6.05, \sigma = 2.88)$) had significantly fewer correct prompts than the TS group ($M = 10.2, \sigma = 1.2$).
The variance of the means was also much larger in in the VR group, which is likely caused by the unfamiliar nature of the VR environment compared to a touchscreen.
Figure \ref{fig:de_correct_stacked} shows the breakdown of the mean result of the prompts for each group and design.

This difference is largely due to subjects not being able to complete the prompt.
Across all groups and designs, very few prompts were completed that were incorrect.
