...
The Touchscreen (TS) group used a \SI{10.1}{\inch} capacitive touch screen with resolution of 1024x600.
...
The disturbance model is based on the model developed in \citet{sweet_identification_1999}.
...
The most direct factor that could contribute to the decreased performance in the tracking task for the VR group is the loss of visual acuity in the tracking task display due to the resolution of the VR head-mounted display.
...
\subsubsection{Number of Correct Prompts}
...
The TS group performed closer to their performance in the Keypad design, however, with only about 1 fewer prompts correct on average.
...
Across all groups and designs, between one and two prompts on average were incorrectly completed in each trial.
...
Throughout the experiment the subjects performed a few trials with only the tracking task, instead of both the tracking task and the prompting task.
...
The NASA-TLX workload scores also showed an interaction effect between Group and Design.
...
However, like the number of correct prompts, the TLX score was found to be only marginally significant for the TS group, rating the Keypad at 61.5 compared to the Edgekey at 69.2.
