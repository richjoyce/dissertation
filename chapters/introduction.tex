...
Our motivation comes from the design of an airplane or spacecraft cockpit, but the key motivating factors and solutions provided by our tool are not limited to the aerospace domain.
Any safety-critical domain which requires a human-machine interface will necessitate the use of human-in-the-loop evaluations of the design of that interface.
The R3C system could be used in the design process of many safety-critical interfaces such as those used for robotic surgery, nuclear control systems, air traffic control, or even cockpits for trains or ships.
The use of the R3C system will enable human-in-the-loop evaluations to occur earlier and more cost effectively.
The term cockpit, used throughout this work, could refer to any `safety-critical human-machine interface`.
...
We solve this by providing the tactile feedback using 3D printed cockpit interfaces co-located with the virtual environment, a technique commonly called ``passive haptics''.
Cockpit interfaces can be any instrument, control, button or switch.
In this work, we refer to them generically as an `instrument'.
...
Their report focused on virtual button interaction using a hand tracker, using a similar collision detection method to activate buttons.
They found that the size of the collision volume for a virtual button did not impact the interaction, but more realistic hand avatar increased efficiency.
...
As well as the quantitative measures of the performance of our system we investigate, we also aim to test our virtual environment as a design tool (Chapter~\ref{chap:de_exp}).
...
During the design of a cockpit, it is important to measure the workload of a pilot to ensure that they are given manageable tasks to perform.
